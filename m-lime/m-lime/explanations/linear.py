...
from density_lime.explanations.models.linear import RidgeMod, HuberRegressorMod, SGDRegressorMod
...
class ExplainLinear(object):
    ...
    def __init__(self, model_predict, density, linear_model='HuberRegressor', linear_param={}, random_state=None, verbose=False):
        """
        ...
        self.local_model_name = linear_model
        if isinstance(linear_model, str):
            if linear_model in standard_linear_models:
                self.local_model = None
                self.local_model = standard_linear_models[self.local_model_name](**linear_param)  
            else:
                raise Exception(
    ...
    def explain_instance(self, x_explain, r=None, class_index=0, n_samples=2000, tol=0.0001, mini_local_batch_max=2000):
        """
        ...
        for step in range(mini_local_batch_max):
        ...
        if diff > tol:
            warnings.warn('Convergence tolerance (tol) was not achieved!'
            +f'Current difference in the importance {diff}/{tol}')
