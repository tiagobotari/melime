...
class KDEGen(GenBase):
    ...
    def sample_radius(
            self, x_exp, r=None, n_min_kernels=10, n_samples=1, random_state=None,
            vec_bandwidth=None, vec_r=None,
            bandwidth_from_local=False, bandwidth=None, include_explain=False
    ):
        ...
        #     _, data[condition] = select_from_box(x_exp, data, vec_r, r)
        #
        #     if bandwidth_from_local:
        #         distances = []
        #         for x_i in data:
        #             distance, _ = self.manifold.tree_.query(x_i.reshape(1, -1), k=2)
        #             distances += [distance[0][1]]
        #         bandwidth = np.mean(distances) * 0.5

        if include_explain:
            data = np.r_[data, x_exp]
        if data.shape[0] == 0:
        ...
        if self.manifold.kernel == "gaussian":
            # if vec_bandwidth is not None:
            #     vec_bandwidth = np.array(vec_bandwidth)
            #     ii, counts = np.unique(i, return_counts=True)
            #     cov = np.diag(vec_bandwidth ** 2)  # * self.bandwidth
            #     x = np.empty(shape=(0, data.shape[1]))
            #     for j, count in zip(ii, counts):
            #         x = np.r_[x, rng.multivariate_normal(data[j], cov, size=count)]
            #     return np.atleast_2d(x)
            # else:
            return np.atleast_2d(rng.normal(data[i], bandwidth))
            ...
            correction = gammainc(0.5 * dim, 0.5 * s_sq) ** (1.0 / dim) * bandwidth / np.sqrt(s_sq)
